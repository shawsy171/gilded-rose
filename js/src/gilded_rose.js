...
function update_quality() {
  ...
  const getUpdateValues = (item) => {
    ...
    const sulfurasValues = { sellInUpdateValue: 0 }
    const conjuredValues = { qualityUpdateValue: -2 }
    const ageBrieValues = { qualityUpdateValue: 1 }
    const backstageValues = (() => {
    ...
    const updateValues =
      sulfurasRegex.test(item.name) ? sulfurasValues
      : conjuredRegex.test(item.name) ? conjuredValues
      : item.name === AGED_BRIE ? ageBrieValues
      : backstageRegex.test(item.name) ? backstageValues
      : itemValues
